...
def find_best_combination_1(actions, budget):
    ...
    items = []
    for action in actions:
        if float(action.cost) <= 0 or int(action.cost) <= 0:
            continue
        else:
            item = (int(action.cost), float(action.benefit_percent) * action.cost / 100)
            items.append(item)
    n = len(items)
...
if __name__ == "__main__":
    ...
    max_value, total_cost, best_combo =find_best_combination_1(actions, 500)
